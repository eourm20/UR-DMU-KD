...
if __name__ == "__main__":
    ...
    normal_train_loader = data.DataLoader(
        XDVideo(root_dir = args.root_dir, mode = 'Train',modal = None, num_segments = args.num_segments, len_feature = args.len_feature, is_normal = True),
            batch_size = args.batch_size,
            shuffle = True, num_workers = args.num_workers,
            worker_init_fn = worker_init_fn, drop_last = True)
    abnormal_train_loader = data.DataLoader(
        XDVideo(root_dir = args.root_dir, mode='Train',modal = None, num_segments = args.num_segments, len_feature = args.len_feature, is_normal = False),
            batch_size = args.batch_size,
            shuffle = True, num_workers = args.num_workers,
            worker_init_fn = worker_init_fn, drop_last = True)
    test_loader = data.DataLoader(
        XDVideo(root_dir = args.root_dir, mode = 'Test',modal = None, num_segments = args.num_segments, len_feature = args.len_feature),
            batch_size = 1,
    ...
    criterion = LossComputer(HPLoss_w=args.HPLoss_w)
    ...
    for step in tqdm(
        ...
        if test_info["auc"][-1] > best_auc:
            ...
            utils.save_best_record(test_info, 
                os.path.join(args.root_dir, "XD_ALL(HPE)5_best_record.txt"))
