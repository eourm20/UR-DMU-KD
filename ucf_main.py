...
if __name__ == "__main__":
    ...
    task = clearml.Task.init(project_name="UR-DMU-HPE", task_name="KD25_dark", task_type=Task.TaskTypes.training)
        ...
        if step % 10 == 0 and step >= 10:
            ...
            if test_info["auc"][-1] > best_auc:
                ...
                utils.save_best_record(test_info, 
                    os.path.join(config.output_path, "ucf_KD25_dark_best_record.txt"))

                torch.save(student_net.state_dict(), os.path.join(args.model_path, \
                    args.model_file.split('<')[0]+"_best.pkl"))
            if step == config.num_iters:
                utils.save_best_record(test_info, 
                    os.path.join(config.output_path, "ucf_KD25_dark_last_record_{}.txt".format(step)))
